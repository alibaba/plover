...
class AppHelper {
  ...
  constructor(rd, viewRender, navigator, app) {
    ...
    this.assetsAware = settings.assetsAware;
  ...
  view(url, data) {
    ...
    if (data) {
      query = Object.assign({}, route.root.query, data);
    }
   ...
   */
  control(url, data) {
    assert(url, 'app.control(url) - url required');

    logger.debug('try render control: %s', url);

    const prd = this.rd;
    const route = RouteInfo.parse(prd.route, url);
    route.type = 'control';
    route.query = prd.route.query;
    ...
    let cdata = prd.data;
    if (data) {
      cdata = Object.assign({}, prd.data, data);
    }
    ...
    const gen = this.viewRender.render(rd);
    return renderChild(prd, gen, url, this.development);
  }
