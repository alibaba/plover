...
class ViewRender {
  ...
  * render(rd, ctx, incept) {
  ...
  include(rd, name, data) {
    const view = resolveView(this, rd, name);
    if (!view) {
      const url = rd.route.module + ':' + name;
    ...
    if (is.promise(content)) {
      if (rd.async) {
        logger.debug('include[promise]: %s, %o', name, data);
        return content.then(output => {
          return new SafeString(output);
        });
      }
      logger.debug('include[async]: %s, %o', name, data);
      const defer = content.then(output => {
        return { content: output };
      });
      return RenderHelper.renderAsync(rd, defer, view, this.development);
    }
    logger.debug('include %s, %o', name, data);
...
function* renderContent(self, rd, view) {
  ...
  // 标识是否支持异步渲染
  // @see core/render-helper.renderAsync
  rd.async = o.async;
 ...
 * 解析View
 */
function resolveView(self, rd, name) {
  const viewRoute = RouteInfo.parse(rd.route, name);
  const info = rd.route.module === viewRoute.module ?
      rd.info : self.moduleResolver.resolve(viewRoute.module);
