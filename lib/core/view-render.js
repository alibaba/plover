...
function* renderContent(self, rd, view) {
  ...
  const path = view.root + '/' + view.template;

  // 渲染器
  const o = compileTpl(self, path);
  ...
  if (is.generator(content)) {
    content = yield* content;
  } else if (is.promise(content)) {
    content = yield content;
  }
 ...
 * @return {Function}
 */
function compileTpl(self, path) {
  const cached = renderCache.get(path);
  if (cached) {
    return cached;
  }

  const ext = pathUtil.extname(path).substr(1);   // remove prefix dot
  const engine = self.engines[ext];
  if (!engine) {
    throw new Error('template engine not found: ' + ext);
  }
