...
function* render(self, rd, view, ctx, incept) { // eslint-disable-line
  ...
  // 渲染内容
  let content = yield* renderContent(self, rd, view);
  ...
  return { content: content, assets: rd.assets };
}
...
function* renderContent(self, rd, view) {
  const context = createViewContext(self.app, rd);
 ...
 * 解析View
 */
function resolveView(self, rd, url) {
  const viewRoute = RouteInfo.parse(rd.route, url);
 ...
 * 创建模板上下文
 */
function createViewContext(app, rd) {
  const ret = {
    $: rd.helpers,
    self: rd.data,
    json: JSON.stringify,

    query: rd.route.query,
    route: rd.route,
  ...
  for (const name of Object.keys(app.helpers)) {
    if (!(name in data)) {
      defineViewHelper(rd, ret, name);
    }
  }
...
function defineViewHelper(rd, ret, name) {
  Object.defineProperty(ret, name, {
    get: function() {
      return rd.helpers[name];
    }
  });
}
