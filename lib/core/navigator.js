...
function resolveModule(self, route) {
  ...
  const module = loadModule(self, info.path);
  if (!module) {
    return null;
  }
...
function loadModule(self, path) {
  ...
  // 如果是类, 暂时先草草构造一个对象, 后续再看如何规划Controller类
...
function* navigateForResult(self, rd, ctx, result) {
  ...
  // 渲染，需要执行渲染拦载器
  result = yield* self.viewRender.render(rd, ctx, true);
  return yield* tryRenderLayout(self, rd, result);
}
...
function* tryRenderLayout(self, rd, result) {
  ...
  if (result && ('content' in result) &&
      !route.navigate && !route.parent && rd.layout.enable) {
...
function* renderLayout(self, contentRoute, layout, result) {
  ...
  Object.assign(rd.data, layout.data);
  ...
  setContent(rd, result);

  return yield* self.viewRender.render(rd, ctx, true);
}


/*
 * 设置内容，布局中可以通过`content`引用到内容
 */
function setContent(rd, result) {
  rd.assets = result.assets;
  rd.data.content = new SafeString(result.content);
}
