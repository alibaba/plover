...
class ViewRender {
  ...
  * render(rd, ctx) {
    ...
    return yield* render(this, rd, view, ctx);
  }
  ...
  include(rd, name, data) {
    ...
    if (data && Object.keys(data).length) {
      data = assign({}, state.data, data);
      context = new HelperContainer(rd, this, this.navigator, this.app);
      assign(context, data);
    } else {
      data = state.data;
    }
 ...
 * 具体的渲染流程
 */
function* render(self, rd, view, ctx) { // eslint-disable-line
  // beforeRender
...
function attachAssets(self, rd, view) {
  ...
  const type = rd.route.type === 'layout' ? 'layout' : 'default';
  const bag = rd.assets[type] ||
      (rd.assets[type] = { css: [], js: [] });

  view.css && bag.css.push({
    route: RouteInfo.parse(view.route, view.css)
  });

  view.js && bag.js.push({
    route: RouteInfo.parse(view.route, view.js)
  });
}
