...
exports.renderAsync = function(rd, defer, url, development) {
  ...
  const index = depends.length;

  // 对Promise异常分支进行包装，避免子模块渲染影响当前模块渲染
  defer = exports.ensure(defer, url, development);
  ...
  if (rd.async) {
    ...
    // render child时还需要合并资源
    // @see exports.renderChildren()
    return defer.then(o => {
      // 允许延迟计算
      ...
      if (typeof o.content === 'string') {
        o.content = new SafeString(o.content);
      }
      return o.content;
    });
  }

  // 标识有异步子模块，后续会采用字符串查找和替换方式渲染子模块
  // @see exports.renderChildren & mergeDepends
  rd.deferred = true;
  const output = PLACE_HOLDER + toFixed(index);
  depends[index] = defer;
  return output;
};
...
exports.renderChildren = function* (rd, content) {
  ...
  logger.debug('render children using string replace: %s', rd.route.url);

  const results = yield rd.depends;
